<?php 
    ...
    */
    class Database
    {
        ...
        {   //kết nối csdl
            $this->link = mysqli_connect("localhost","root","","btl") or die ();
        ...
        public function insert($table, array $data)
        {
            ...
            foreach($data as $field => $value) {
                if(is_string($value)) {
                    $values .= "'". mysqli_real_escape_string($this->link,$value) ."',"; //làm dữ liệu an toàn khi gửi đến sql
                } else {
...
//Lấy ds sản phẩm mới
        public function fetchsql( $sql )
        {
            ...
            if( $result)
            {
                while ($num = mysqli_fetch_assoc($result))//sẽ tìm và trả về một dòng kết quả của một truy vấn MySQL nào đó dưới dạng một mảng kết hợp.
                {
                    $data[] = $num;
                }
            }
...
//Kiểm tra xem danh mục đã tồn tại sản phẩm chưa mới được phép xóa----
    ...
    // phân trang trong danh mục( bảng, câu lệnh, tổng số bản ghi, page, bao nhiêu sản phẩm/ trang, có phân trang ko? )
